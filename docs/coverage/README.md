...
- LIVE STUDIO CHECK PASSED: assets/threewave chunks synced into
  ServerStorage.QuakeAssets.threewave (184 chunks via localhost HTTP),
  attrs engine=qw gamedir=threewave startmap=ctf2m3 teamplay=419 — console
  shows the full stack (id1 pak0/pak1 → threewave pak0/pak1/pak2), "You
  have been assigned to Blue team", world "Spill the Blood"; red-flag grab
  and a live capture ("BLUE 15") evidenced under evidence/qw-ctf2m3-*.jpg.
  Regression smokes after: QW dm3 (chunk-asset qwprogs fallback path) and
  NQ start map both boot clean; attrs restored to defaults.
