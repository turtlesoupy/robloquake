...
| SVC_DirectConnect | `qwsv.allocClient` + `qwsv.wireConnect` (qwsv.luau:460,502) + `connectPlayer` (qwserver.luau) | VERIFIED | test_qw_loopback: wireConnect → handshake completes; slot setup + SetNewParms spawn parms match the C core. Challenge/qport/rate/IP checks dropped (SUBSTITUTED by platform auth). delta-ruled: ACCEPT (2026-07-07) — Roblox authenticates players before any packet arrives; challenge/qport/rate/IP checks defend a hostile UDP surface that does not exist here. | `lune run tests/test_qw_loopback.luau` |
...
| SV_PreSpawn_f | `qwsv.prespawnF` (qwsv.luau:908) | VERIFIED | test_qw_loopback "baselines received (>20)". Delta: C paginates the signon across several `cmd prespawn` round-trips; port copies the whole signon buffer in one reliable write (transport has no 1450-byte packet limit) and errors on overflow instead of chunking. delta-ruled: ACCEPT (2026-07-07) — transport-inherent; caveat: a giant custom map could hard-error on signon overflow where C would chunk. | `lune run tests/test_qw_loopback.luau` |
...
| SV_Begin_f | `qwsv.beginF` (qwsv.luau:1044) | VERIFIED | test_qw_loopback "handshake completed: server spawned the client"; ClientConnect/PutClientInServer run exactly once here. Delta: no model-checksum ("check") anti-cheat, no unpause-on-begin counters. delta-ruled: ACCEPT (2026-07-07) — pmodel/emodel checksum anti-cheat is pointless with server-published assets; C's "counters" are netchan rate statistics (sv_user.c:476-479) that don't exist on this transport (the svc_setpause notify itself IS ported in beginF). | `lune run tests/test_qw_loopback.luau` |
...
| SV_ShowServerinfo_f | `serverinfo` user command → `qwsv.infoPrint` (20-column keys; sorted for stable order — recorded delta vs C's raw string order; delta-ruled: ACCEPT (2026-07-07) — deliberate cosmetic sort, output content identical) | VERIFIED | test_qw_loopback: the command answers the requesting client with the info table over the wire (PRINT_HIGH). | `lune run tests/test_qw_loopback.luau` |
...
| SV_ExecuteUserCommand | `qwsv.executeUserCommand` (qwsv.luau:1203) + `tokenize` | VERIFIED | Drives the whole loopback handshake (new/soundlist/modellist/prespawn/spawn/begin). Delta: unknown commands only dprint (C prints to client). delta-open: FIDELITY.md "Open" (2026-07-07 delta triage) | `lune run` full sweep (harness-cited; pin the exact test in the burn-down) |
...
| AddLinksToPmove / AddAllEntsToPmove | `addLinksToPmove` areanode walk (qwsv.luau:654) | VERIFIED | test_qwsv movement + loopback prediction run through it. MAX_PHYSENTS=32 kept; physent order → `onground` index mapping handled via `physentEdicts`. Delta: a SOLID_BSP edict with a non-brush/missing modelindex is omitted from physents with a one-shot server print where C Sys_Errors the whole server (sv_user.c:1276 indexes sv.models unchecked). delta-ruled: ACCEPT (2026-07-07) — staying up beats C's crash; the warn keeps it from masking QC bugs silently. | `lune run tests/test_qwsv.luau` |
...
| SV_PostRunCmd | `qwsv.postRunCmd` (qwsv.luau:754) | VERIFIED | test_qwsv shotgun: PlayerPostThink fired the weapon (shell consumed, guncock sound). SV_RunNewmis folded in via physicsToss at 0.05s. SpectatorThink branch untested. coverage-debt: docs/coverage/delta-triage-2026-07-07.md | `lune run tests/test_qwsv.luau` |
| SV_ExecuteClientMessage | `qwsv.executeClientMessage` (qwsv.luau:1243) | VERIFIED | test_qw_loopback: full clc stream (nop/delta/move/stringcmd) with net_drop fill-in from lastcmd/oldest/oldcmd, ping bookkeeping via frame ack. Deltas: move checksum byte read but not validated (authenticated transport); msec-overuse cheat detection (sv_user.c timer checks) absent. delta-ruled: ACCEPT (2026-07-07) — both are anti-cheat against spoofed/hostile UDP; the transport is authenticated. clc_tmove spectator-only teleport ported, untested. coverage-debt: docs/coverage/delta-triage-2026-07-07.md | `lune run tests/test_qw_loopback.luau` |
...
| SV_WritePlayersToClient | `qwsv.writePlayersToClient` (qwsv.luau:1525) | VERIFIED | test_qwsv hand-parses svc_playerinfo: self omits PF_MSEC/PF_COMMAND, origin within 1/8 quantization; spectator/spec_track masks ported (untested branches). coverage-debt: docs/coverage/delta-triage-2026-07-07.md | `lune run tests/test_qwsv.luau` |
...
| SV_UpdateClientStats | `qwsv.updateClientStats` (qwsv.luau:1328) | VERIFIED | test_qw_loopback: health/shells stats mirror server, ammo drops after firing; spectator spec_track passthrough ported (untested); sigil bits <<28 into STAT_ITEMS. coverage-debt: docs/coverage/delta-triage-2026-07-07.md | `lune run tests/test_qw_loopback.luau` |
...
| SV_UpdateToReliableMessages | `qwsv.updateToReliableMessages` (qwsv.luau:2411-2476) | VERIFIED | Full port: sendinfo resend, frag rebroadcast, per-client entgravity/maxspeed pickup with svc_entgravity/svc_maxspeed writes (per sv_send.c:648-661), reliable_datagram + datagram fan-out. Frag path: loopback "frag change rebroadcast". Fan-out path: conSay/say prints traverse reliable_datagram to the spawned client. (Stale "partial" text corrected by the 2026-07-07 delta triage.) | `lune run tests/test_qw_loopback.luau` |
| SV_SendClientMessages | `qwsv.sendClientMessages` + `qwsv.replyToClient` (qwsv.luau:1448,1466) | VERIFIED | Loopback: strict 1:1 packet exchange (send_message flag) keeps sequence spaces aligned for delta/prediction. Delta: no rate/choke logic (chokecount always 0 — transport does not drop), spectator slower update path absent. delta-ruled: ACCEPT (2026-07-07) — rate/choke is bandwidth management for lossy UDP; spectators simply get full-rate updates (strictly more data, no gameplay change). | `lune run` full sweep (harness-cited; pin the exact test in the burn-down) |
...
`qwphys.physics` frames and runs live play; door travel/return is tested below (SV_Physics_Pusher), but
plat and item toss-lifecycle behavior beyond that remains untested here.
coverage-debt: docs/coverage/delta-triage-2026-07-07.md
...
| SV_CheckWaterTransition | `checkWaterTransition` (qwphys.luau:740) | VERIFIED | test_qwsv toss path (dry transition only; water entry/exit awaits the water truth course — the pmove water course covers player swimming, not toss entities). coverage-debt: docs/coverage/delta-triage-2026-07-07.md | `lune run tests/test_qwsv.luau` |
...
| SV_RunEntity | movetype dispatch in `sv_phys.physics` (qwphys.luau:872) | VERIFIED | test_qwsv "SV_RunEntity flew the spike": a MOVETYPE_FLYMISSILE spike advances through the movetype dispatch while client slots stay excluded (movement checks would break otherwise). C's per-entity `lastruntime` throttle IS implemented (qwphys.luau:888-896, float compare as C) with the newmis double-run skip (qwphys.luau:951-953) — the former "throttle absent" delta was stale (corrected by the 2026-07-07 delta triage). | `lune run tests/test_qwsv.luau` |
...
| SV_Physics | `sv_phys.physics` (qwphys.luau:849) | VERIFIED | test_qwsv/test_qw_loopback every tick; force_retouch decrement; sv.time advanced by qwsv.frame, not here — matches C split. QC `frametime` global written before StartFrame like sv_phys.c:907 (was missing until 2026-07-07 — stock qw-qc never reads it, but mod progs can; only runCmd wrote it). Deltas: sv_mintic frame-skip dropped (physics runs per heartbeat) — delta-ruled: ACCEPT (2026-07-07) — heartbeat-driven physics is the port architecture and player movement stays per-cmd like C; reopen if a mod shows tick-granularity-sensitive behavior. SV_RunNewmis's 0.05 frametime stamp restored after the newmis run instead of leaking to later same-frame entities (sv_phys.c:874 never restores) — delta-ruled: ACCEPT (2026-07-07) — the leak is a global-mutation accident, and with per-heartbeat dt reproducing it would distort far more entities than C's ~30Hz frames ever did. | `lune run tests/test_qw_loopback.luau`; `lune run tests/test_qwsv.luau` |
...
| Function | Port | Status | Evidence / Delta | How to verify |
|---|---|---|---|---|
| ClientReliableCheckBlock / ClientReliable_FinishWrite | `qwsv.reliableCopy` backbuf equivalent (qwsv.luau:563-564, 1790-1816) for the fan-out path | SUBSTITUTED | The C backbuf queues reliable data while a netchan reliable is in flight; on the reliable+ordered Roblox transport direct writers append straight to `client.netchan.message` (8 KB) with a size check, while the reliable_datagram fan-out queues through a backbuf equivalent (`qwsv.reliableCopy` + `client.backbuf`/BACKBUF_CAP) so reliable-by-contract data is never silently dropped. (Text updated 2026-07-07 delta triage — the pure "no backbuf" description was stale.) | — (substitution; verify justification still holds) |
| ClientReliableWrite_Begin/Angle/Angle16/Byte/Char/Float/Coord/Long/Short/String/SZ | direct `msg.write*` into `client.netchan.message` (throughout qwsv.luau) | SUBSTITUTED | Same wire bytes; direct writers skip the backbuf indirection (the reliable_datagram fan-out path uses the `qwsv.reliableCopy` backbuf — see previous row); loopback reliable traffic (prints, stats, setangle) verifies the stream itself. | — (substitution; verify justification still holds) |
...
| SV_Floodprot_f / SV_Floodprotmsg_f | hardcoded values in `say` handler | SUBSTITUTED | Flood protection on (4/4s/10s) but not tunable; C default is off until configured. delta-ruled: ACCEPT (2026-07-07) — deliberate BEHAVIOR CHANGE: flood protection is ON by default where C ships OFF; chat flooding is blocked out of the box. | — (substitution; verify justification still holds) |
...
| PF_localcmd (46) | qwbuiltins.luau:605 | VERIFIED | test_qwbuiltins: "changelevel e1m2" routes to svr.changelevelTo (consumed by the QW boot since b98aa9a). Delta: only changelevel/restart routed; others logged. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage) | `lune run tests/test_qwbuiltins.luau` |
...
| PF_aim (44) | qwbuiltins.luau:533 | VERIFIED | test_qwbuiltins: with sv_aim 2 (the QW default — assist off) returns v_forward unchanged, which is the only branch stock QW play reaches. Assist branch remains untested (dead under default cvars). coverage-debt: docs/coverage/delta-triage-2026-07-07.md | `lune run tests/test_qwbuiltins.luau` |
...
| PF_infokey (80) | qwbuiltins.luau:828 | VERIFIED | test_qwbuiltins: world reads serverinfo, client reads userinfo, world falls back to localinfo when serverinfo lacks the key + serverinfo-wins precedence (C's PF_infokey order); end-to-end consumer = Rocket Arena's fraglimit rotation (S6). C's synthetic keys ARE implemented: `ip` (stable UserId transport identity) and `ping` via calcPing (qwbuiltins.luau:872-878) — the former "absent" delta was stale (corrected by the 2026-07-07 delta triage). | `lune run tests/test_qwbuiltins.luau`; `lune run tests/test_scenario_ra.luau` |
...
| GetEdictFieldValue | field lookups via qwdefs `ent` table | SUBSTITUTED | Static name→offset resolution replaces per-call cached lookup. The `gravity`/`maxspeed` optional-field pickup that C does with it IS implemented (qwsv.luau:2438-2454, per sv_send.c:648-661; see SV_UpdateToReliableMessages) — the former "missing" note was stale (corrected by the 2026-07-07 delta triage). | — (substitution; verify justification still holds) |
...
| Netchan_Process | `qwnetchan.process` (qwnetchan.luau:57) | VERIFIED | Stale/duplicate rejection, drop_count (net_drop) feeding SV_RunCmd's dropped-cmd replay — loopback runs with 1 tick latency. Delta: 32-bit sequence wrap unhandled (~2.3y at 60Hz). delta-ruled: ACCEPT (2026-07-07) — wrap horizon is ~2.3 years of continuous uptime; unreachable on a Roblox server. | `lune run` full sweep (harness-cited; pin the exact test in the burn-down) |
...
|---|---|---|---|
| `svr.soundLog` / `svr.printLog` / `client.prints` mirrors | qwsv.luau:160-188, 224 | Justified: offline tests assert sounds/prints without a client (test_qwsv guncock check reads soundLog). Unbounded growth in long sessions — soundLog/printLog are never trimmed (fraglog is capped at 512). delta-open: FIDELITY.md "Open" (2026-07-07 delta triage) |
| `svr.startParticle` logging into soundLog | qwsv.luau:220 | Stub behind the port's `def(48, "particle")` — NOT a QW C feature: QW's builtin slot 48 is PF_Fixme (reference/quake-c/QW/server/pr_cmds.c:1673) and no svc_particle/SV_StartParticle exists anywhere in the QW server tree. A port-side NQ-ism kept for mod tolerance. delta-ruled: ACCEPT (2026-07-07) — not a QW C feature; port-side addition, no wire path to build. |
...
| `svr.changelevelTo`/`changelevelIssued` | qwsv.luau:242 | Justified as the PF_changelevel latch (C calls Cbuf_AddText "map ..."); consumed by the QW boot (qwserver.luau:260-262, changelevel driver since b98aa9a — see SV_Map_f). The "no QW consumer exists yet" note was stale (corrected by the 2026-07-07 delta triage). |
...
3. ~~SV_UpdateToReliableMessages partial~~ — fully ported 2026-07-04 (frag rebroadcast
   loopback-verified).
4. ~~PF_lightstyle's live-broadcast branch is dead (`svr.ss_active` is nil)~~ — fixed: the
   broadcast is live behind `state == 2` with netchan reliable writes (qwbuiltins.luau:519-525);
   loopback-verified ("runtime PF_lightstyle broadcast reached the client", see the PF_lightstyle row).
5. ~~changelevel is latched but never consumed in the QW boot~~ — fixed: `svr.changelevelTo` is
   consumed by the QW boot (qwserver.luau:260-262, changelevel driver since b98aa9a; see SV_Map_f).
