...
| SV_Physics | `sv_phys.physics` (qwphys.luau:849) | VERIFIED | test_qwsv/test_qw_loopback every tick; force_retouch decrement; sv.time advanced by qwsv.frame, not here — matches C split. QC `frametime` global written before StartFrame like sv_phys.c:907 (was missing until 2026-07-07 — stock qw-qc never reads it, but mod progs can; only runCmd wrote it). Deltas: sv_mintic frame-skip dropped (physics runs per heartbeat) and SV_RunNewmis's 0.05 frametime stamp is restored after the newmis run instead of leaking to later same-frame entities — both pending delta-ruled annotations. | `lune run tests/test_qw_loopback.luau`; `lune run tests/test_qwsv.luau` |
