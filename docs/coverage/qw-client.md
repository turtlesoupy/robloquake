...
| CL_ParseClientdata | head of `parseServerMessage` | PENDING | DEMOTED (evidence not re-runnable/checked-in; re-earn with a test or docs/coverage/evidence/ screenshot): parsecount = incoming_acknowledged, receivedtime, latency drift (drift-up 0.001 rule). Prediction convergence (<1 unit) depends on it. | TBD: write test or tools/verify script + evidence capture |
...
| CL_MuzzleFlash | `svc_muzzleflash` → tempEntities type -1 → qwclient `handleTempEntity` | PENDING | Dlight 18 units forward, radius 200+rand(31), die +0.1, minlight 32 — matches C; self uses simorg/viewangles, others use frame playerstate. No visual check recorded. | TBD: write test or tools/verify script + evidence capture |
...
| CL_ParseDelta | `qwents.parseDelta` | PENDING | DEMOTED (evidence not re-runnable/checked-in; re-earn with a test or docs/coverage/evidence/ screenshot): qwents: "moved origin applied", "new entity fields", U_MOREBITS byte, all U_ field reads. | TBD: write test or tools/verify script + evidence capture |
...
| CL_EmitEntities | `relinkEntities` orchestration in heartbeat | PENDING | DEMOTED (evidence not re-runnable/checked-in; re-earn with a test or docs/coverage/evidence/ screenshot): Live: 8 ents at dm3 yard spawn render; keyed create/destroy replaces the visedict rebuild. | TBD: write test or tools/verify script + evidence capture |
...
| CL_ParseTEnt | `qwcl` `parseTempEntity` + qwclient `handleTempEntity` | PENDING | All 14 TE_ types wired with C particle counts/colors, randomized spike ric sounds, explosion dlight (350/0.5s/decay 300). Backlog 115a438 lists tent effects among fixed audit gaps; the recorded live check named view weapon/items, not effects. | TBD: write test or tools/verify script + evidence capture |
...
| V_CalcRefdef | camera block in heartbeat | PENDING | DEMOTED (evidence not re-runnable/checked-in; re-earn with a test or docs/coverage/evidence/ screenshot): Re-verified WITH the HUD present (the prior VERIFIED predated the sbar and missed the vrect occlusion): bob/roll/punch/dead/gib/intermission + vrect view-model placement, screenshot + live projection measurements 2026-07-04. Stair-step smoothing (oldz 80 u/s glide, 12-unit cap) ported 2026-07-04 after a playtest report of harsh stair climbs — it was missing entirely. Deltas: no view_ofs from server, gun bob simplified to forward push, gun lag smoothing (CalcGunAngle) still absent. | TBD: Studio capture — climb the dm3 x=-64 stairs, sample camera z per frame via a tools/verify script, assert the eye glides (no 16-unit jumps) while QW_SimOrg steps |
| DropPunchAngle | `punchangle -= 10*dt`, clamp 0 | PENDING | DEMOTED (evidence not re-runnable/checked-in; re-earn with a test or docs/coverage/evidence/ screenshot): Backlog 2ee4228 note: "QW DropPunchAngle is verbatim (kicks last one frame — authentic QW)". svc_smallkick/bigkick set -2/-4. | TBD: write test or tools/verify script + evidence capture |
...
| Function | Port | Status | Evidence / Delta | How to verify |
|---|---|---|---|---|
| MSG_WriteDeltaUsercmd | `qwents.writeDeltaUsercmd` | PENDING | DEMOTED (evidence not re-runnable/checked-in; re-earn with a test or docs/coverage/evidence/ screenshot): qwents: full round trip (CM_ bits, msec always written). | TBD: write test or tools/verify script + evidence capture |
...
| MSG_WriteAngle16 / MSG_ReadAngle16 | `msg.writeAngle16` / `msg.readAngle16` | PENDING | DEMOTED (evidence not re-runnable/checked-in; re-earn with a test or docs/coverage/evidence/ screenshot): qwents: "cmd angle1/2 round-trips (angle16, mod 360)" — signed-short congruence semantics preserved. | TBD: write test or tools/verify script + evidence capture |
...
| r_sky.c / d_sky.c | shared sky (10Hz image scroll) | SUBSTITUTED | Live-verified under the NQ boot; same module. | — (substitution; verify justification still holds) |
...
| r_part.c: R_LavaSplash / R_TeleportSplash | `particles.lavaSplash` / `.teleportSplash` | PENDING | Same. | TBD: write test or tools/verify script + evidence capture |
| r_part.c: R_RocketTrail (all 7 trail types) | `particles.rocketTrail` | PENDING | QW trail-type mapping (rocket/grenade/gib/zomgib/tracer1-3 by model flags) wired in `relinkEntities`. | TBD: write test or tools/verify script + evidence capture |
