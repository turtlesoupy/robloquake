...
| Function | Port | Status | Evidence / Delta | How to verify |
|---|---|---|---|---|
| Draw_Init | confont/textures lazy init | VERIFIED | conchars/wad pic pipelines feed every committed capture (console text, sbar pics, menu plaques); lazy init is the only delta from C. FIDELITY FIX 2026-07-04 (playtest "more dark textures"): live EditableImage bindings are a scarce pool (~128) and binds past the cap render BLACK — the font is now ONE 128x128 sheet addressed by ImageRectOffset (was one image per glyph, 40 live) and sbar/intermission pics pack into one shared 512 page (textures.newImagePage/pageAlloc); e1m1 steady state dropped 140 -> 77 distinct images. | Any Play capture with the console open |
...
| R_NewMap | onServerInfo world build (worldmesh.build, deferred one Heartbeat) | VERIFIED | tests/test_loopback.luau + test_changelevel.luau world loads; rebuild-starvation fix (user playtest "black square"): 4-rebuild gauntlet leaves 61 world parts + animated style-10 alcove region, flicker pair committed ([evidence/nq-e1m1-flicker-bright.jpg](evidence/nq-e1m1-flicker-bright.jpg), [-dark.jpg](evidence/nq-e1m1-flicker-dark.jpg), [.txt](evidence/nq-e1m1-flicker.txt)). FIDELITY FIX 2026-07-04: non-pow2 WORLD textures (e1m1 ships 21) rendered black on MeshParts; worldmesh now resamples them up to pow2 like GLQuake's GL_Upload8 (textures.resamplePixelsPow2 — padding would break tiling UVs), covering world faces, brush submodels, b_*.bsp boxes, anim frames and fullbright overlays. test_render_misc resample battery + e1m1 census. | `lune run tests/test_changelevel.luau`; `lune run tests/test_render_misc.luau`; Studio: tools/verify_meshbudget.luau gauntlet prints PASS |
...
| R_AliasDrawModel | entrender.updateAlias | VERIFIED | FIDELITY FIX 2026-07-04 (user playtest): non-pow2 EditableImages render BLACK as MeshPart textures — every alias skin was a black silhouette in-world; skins now pad to pow2 with UVs scaled (diagnostic sequence in [evidence/nq-black-entities.txt](evidence/nq-black-entities.txt)). The fullbright-skin substitution stands. | Approach any monster; diagnostic sequence per the evidence file |
...
| shared GUI image pages (font sheet + sbar pic page) | confont.luau / hud.luau / textures.newImagePage | platform constraint: ~128 live EditableImage bindings render; the next bind draws BLACK with healthy CPU pixels (verified live: a fresh solid-red image bound black past the cliff). Alias-skin/sprite paging is the remaining stage if campaign maps regress |
