...
| dynamic→FixedSize EditableMesh copy dance, scratch never retained, backoff retries on budget failure | worldmesh.luau / entrender.luau | code comment: dynamic meshes reserve the 60k-vertex max against the memory budget; 2026-07-05 black-texture root cause: ONE retained dynamic (the old "render dynamic" fixed-copy-failure fallback) saturates the budget for the session and images created at the margin zombie (GPU upload silently refused, black forever) — failed builds now destroy the scratch and retry with backoff (~3s), then drop the batch / box-fallback |
| exclusive build slot (one scratch dynamic EM at a time, queued builds drip one per Heartbeat) | buildqueue.luau (worldmesh + entrender attempts run through it) | same root cause: the async mesh APIs yield, so sighting-burst builds ran concurrently, each holding a 60k scratch reservation — the transient saturation where fixed copies fail and images zombie; serialization removes it at the source. tests/test_buildqueue.luau (11 checks) |
| deferred off-thread ent/template builds: in-flight guards, exponential failure backoff, worldBuildGen/renderGen stale-build discard | init.client.luau getModelDef + entity loop + statics + beams + gun; qwclient.luau twins | same root cause, caller side: builds yield, so per-frame re-entry both duplicated concurrent builds (the burst) and hammered retries after failures (worsening the very pressure that failed them); getModelDef no longer yields (template builds kick off task.spawn'd, "pending" second return). Also fixes latent bugs found in the pass: NQ statics never respawned after changelevel (statics_spawned never reset), QW beam pool ents + NQ/QW beam def images leaked across level changes |
