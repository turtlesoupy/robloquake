...
| quiet-window alias mesh regeneration | entrender.luau (queueMeshRegen/drainMeshRegen, both boots by construction) | platform defect, per-ENTITY flavour: each alias ent's own EditableMesh/MeshPart can be born with a silently-failed GPU upload (mixed textured/black siblings of one def). Every alias ent's mesh is rebuilt ONCE ~1.5s after entity creation goes quiet and swapped in place (CFrame/Color/skin preserved, forced re-pose); statics and the gun included. Playtest-statistical acceptance — single clean runs prove nothing. |
