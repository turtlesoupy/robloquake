...
| alias mesh POOL + quiet-window regeneration backstop | entrender.luau (meshPool/prewarmPools/poolAcquire/poolRelease + queueMeshRegen/drainMeshRegen; both boots prewarm after texverify) | platform defect, per-ENTITY flavour: an EditableMesh/MeshPart born during a hot creation stretch can silently lose its GPU upload (mixed textured/black siblings of one def). Mid-play births are ALWAYS hot (backpack drops in the kill's gib burst; nails/gibs reset any quiet clock), so runtime healing alone cannot keep up — instead instances are PRE-BUILT in the post-load settle window (baseline census per model + fixed counts for projectiles/gibs/heads/backpacks; `mesh pools prewarmed: N` in boot logs) and recycled via acquire/release; steady-state play creates no GPU objects. Overflow live-builds are marked suspect and rerolled by the quiet-window regen (live in-place swap, or silent cure while parked). Verified 2026-07-05: e1m1 boot 101 prewarmed, 9 sightings + mid-combat backpack+gib drop all pool-acquired (overflow counter flat), all textured. Watch workspace attrs RQDBG_PoolPrewarmed/RQDBG_PoolOverflow. HARDENED same day after a playtest hole (black armor): a SHARED creation clock (textures.noteCreation/timeSinceCreation, bumped by createImage, worldmesh.build, overflow builds) replaces the alias-only quiet clock; prewarm instances built within 1.5s of any creation are marked suspect and cured while parked (the user plays during the settle window — "quiet" must be measured, not assumed). Playtest-statistical acceptance — single clean runs prove nothing. |
| texverify closed-loop render verification | render/texverify.luau, both boots after eager precache + on new translations | platform defect: editable objects created under load can silently lose their GPU upload and render black with healthy CPU pixels (bisect: predates all recent code; nothing repairs in place, recreation always heals). Detection via CaptureService screenshot -> CreateEditableImageAsync -> ReadPixelsBuffer over a tile grid with a green control tile; zombies recreated from source pixels, identity-gated binds propagate swaps to live ents. `texverify: N images checked` in boot logs. Pure gate textures.sourceHasArt lune-tested. |
| brush template rebuild pass | init.client.luau / qwclient.luau settle tasks + entrender.refreshBrush | platform defect, per-TEMPLATE flavour: brush templates (doors/plats "*N" + b_*.bsp health/ammo boxes) are born in the load burst, and a zombied template renders black through EVERY clone all level (playtest evidence: "all crates of one ammo type black", black health boxes). Post-settle, each template is rebuilt in a measured-quiet moment (shared creation clock), swapped into its def, and live ents re-cloned via entrender.refreshBrush; old templates are RETIRED to a graveyard destroyed only at teardown (the 10Hz anim pumps still hold their images — destroying mid-level would error the pumps). `brush templates rebuilt: N` in boot logs (e1m1: 64). HARDENED after a black-first-switch playtest: the pass now runs BEFORE pool prewarm, PRIORITIZED (b_*.bsp items, then "*N" ascending — buttons/doors the player meets first), gated by a creation COUNTER (foreign creations force fresh 1.5s quiet; the pass's own builds only need 0.6s spacing → ~1.4 templates/s, e1m1 fully healed <60s), and each template is pcalled so a budget-wall failure cannot silently kill the rest. Progress attr RQDBG_TemplatesRebuilt. |
