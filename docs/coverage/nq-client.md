...
| V_CalcRefdef | view.calcRefdef | VERIFIED | test_view.luau composition battery: origin + viewheight + bob + 1/32 nudge, gun origin eye + forward*bob*.4 + z bob, punch added AFTER gun angles (C order), stair glide 80 u/s with the 12-unit cap and airborne snap, STAT_WEAPON/WEAPONFRAME pass-through. Deltas: pitch drift + V_BoundOffsets landed 2026-07-05 (see their rows); still absent: viewsize gun-z fudge, player ent angles not forced to view. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | `lune run tests/test_view.luau` |
