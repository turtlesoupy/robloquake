...
| CL_ParseServerMessage | cl.luau parseServerMessage | VERIFIED | tests/test_loopback.luau full protocol path; FIDELITY.md byte-exact protocol 15 layer; svc_stopsound routed since 2026-07-04 (cl.luau:641-644 → c.onStopSound → sound.stop; see the S_StopSound row — the old read-but-discarded note was stale); svc_cdtrack stored but has no consumer. delta-ruled: ACCEPT (2026-07-07) — CD music tracks were never in the paks (FIDELITY.md "Platform substitutions" CD-music entry; LibreQuake banking is the recorded future option) | `lune run tests/test_loopback.luau` |

## cl_input.c

| Function | Port | Status | Evidence / Delta | How to verify |
|---|---|---|---|---|
| KeyDown / KeyUp | input.setButton via +/- command routing | VERIFIED | Real W keyDown started motion, keyUp stopped it ([evidence/nq-input-menu-battery.txt](evidence/nq-input-menu-battery.txt)). Delta stands: no two-keycode tracking per button. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | Studio: tools/verify_input_nq.luau battery (user_keyboard_input steps documented in the script) |
...
| CL_KeyState | boolean btn() | VERIFIED | Press/release edges observable in the W battery (motion exactly while held, [evidence/nq-input-menu-battery.txt](evidence/nq-input-menu-battery.txt)). Delta stands: no partial-frame press fractions. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | Studio: tools/verify_input_nq.luau battery (user_keyboard_input steps documented in the script) |
...
| Function | Port | Status | Evidence / Delta | How to verify |
|---|---|---|---|---|
| CL_InitTEnts | lazy beamModelDef + soundbank regions | VERIFIED | Bolt model + lightning sound resolved on first LG fire in the live battery ([evidence/nq-lightning-beam.jpg](evidence/nq-lightning-beam.jpg) + [.txt](evidence/nq-lightning-beam.txt)). Delta stands: loaded lazily instead of precached. delta-ruled: ACCEPT (2026-07-07) — port-wide lazy asset model. | Stage per evidence/nq-lightning-beam.txt |
...
| CL_NewTempEntity | beamPool pooled render entities | VERIFIED | Pooled bolt segments visible along the beam ([evidence/nq-lightning-beam.jpg](evidence/nq-lightning-beam.jpg) + [.txt](evidence/nq-lightning-beam.txt)). Delta stands: pooled per model name, 40-segment cap. delta-ruled: ACCEPT (2026-07-07) — pooling model; C's 64-total temp-entity cap never binds in id1 (LG range 600 = 20 segments). | Stage per evidence/nq-lightning-beam.txt |
| CL_UpdateTEnts | init.client updateBeams | VERIFIED | 30-unit segments with random roll marching player-muzzle-to-impact, live while held ([evidence/nq-lightning-beam.jpg](evidence/nq-lightning-beam.jpg) + [.txt](evidence/nq-lightning-beam.txt)). Delta stands: C int-truncates yaw/pitch, port keeps float. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | Stage per evidence/nq-lightning-beam.txt |
...
| V_StartPitchDrift | shared `view.startPitchDrift` (centerview command in init.client) | VERIFIED | test_view drift battery: arms pitchvel at v_centerspeed 500, refused when stopped the same frame (laststop==time guard). centerview now GLIDES per C instead of the old pitch snap. | `lune run tests/test_view.luau` |
| V_StopPitchDrift | shared `view.stopPitchDrift` (any manual pitch motion in the heartbeat) | VERIFIED | test_view: nodrift set, pitchvel zeroed, laststop stamped. Boot calls it on any manual pitch change — C's IN_MouseMove stop under +mlook (recorded delta: keyboard +lookup also stops it here, where C lets them fight). delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | `lune run tests/test_view.luau` |
| V_DriftPitch | shared `view.driftPitch` driven per-frame in the heartbeat (cl.idealpitch is finally consumed) | VERIFIED | test_view: accelerating glide (move uses the OLD pitchvel, += ft*500), exact overshoot clamp at idealpitch (strict >), airborne reset, forward-running re-arm past v_centermove 0.15 at cl_forwardspeed 200. Demo playback resets like C. | `lune run tests/test_view.luau` |
...
| V_cshift_f | `v_cshift` command in init.client (edits cshift_empty {130,80,50,0}; empty contents pick it up) | VERIFIED | [evidence/nq-vcshift.jpg](evidence/nq-vcshift.jpg) + .txt: "v_cshift 0 120 200 90" tints the whole view teal live; bare v_cshift zero-fills (atoi). | Stage per evidence/nq-vcshift.txt |
...
| V_SetContentsColor | init.client view-leaf contents shifts | VERIFIED | [evidence/nq-slime-cshift.jpg](evidence/nq-slime-cshift.jpg) + .txt: full-screen slime tint while submerged, blended with the damage flash from the slime tick. Delta: no CONTENTS_SOLID grey (cannot be seen in play). delta-ruled: ACCEPT (2026-07-07) — solid-contents view is noclip-only, unreachable in play. | Teleport per evidence/nq-slime-cshift.txt, capture, compare |
| V_CalcPowerupCshift | view.powerupShift (shared; init.client delegates) | VERIFIED | test_view battery vs view.c:442: quad(0,0,255,30) > suit(0,255,0,20) > invisibility(100,100,100,100) > invulnerability(255,255,0,30) priority chain, combined-items precedence, none->nil. FIDELITY FIX 2026-07-07: the IT_QUAD/IT_SUIT/IT_INVULNERABILITY bit constants were rotated (quad flashed yellow, suit blue, pentagram green) AND test_view asserted the same wrong bits — both re-grounded against quakedef.h:157-160 (wiring-audit find; a C-truth test is only as good as its truth). | `lune run tests/test_view.luau` |
| V_CalcBlend | view.calcBlend (shared; init.client delegates) | VERIFIED | test_view hand-computed truths: single shift passes through, two-shift alpha accumulates a += a2*(1-a) with colors weighted a2/a, empty -> transparent. Delta stands: the Roblox layer caps the on-screen alpha at 0.85. delta-ruled: ACCEPT (2026-07-07) — deliberate readability cap on the GUI tint. Live composite: nq-slime-cshift evidence. | `lune run tests/test_view.luau` |
...
| angledelta / CalcGunAngle | gunangles = (-pitch, yaw, 0) | SUBSTITUTED | Gun locked to the view: the C damped yaw/pitch lag (angledelta smoothing) is not ported; the base mapping (-pitch, yaw, punch excluded) is asserted in test_view.luau. Expiry: port the damped lag if playtesting reports the gun feeling glued/stiff during fast turns. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | `lune run tests/test_view.luau` (base mapping) |
| V_BoundOffsets | `view.boundOffsets` inside calcRefdef | VERIFIED | test_view: +/-14 lateral and -22/+30 vertical clamps, normal eye passes through untouched (bob+viewheight never reach the bounds — kept for C shape; scr_ofs cvars remain fixed 0). | `lune run tests/test_view.luau` |
| V_AddIdle | `view.addIdle` (intermission forces scale 1; v_idlescale 0 otherwise like C) | VERIFIED | test_view: exact cvar constants — pitch sin(t*1)*0.3, yaw sin(t*2)*0.3, roll sin(t*0.5)*0.1, scaled by idlescale. | `lune run tests/test_view.luau` |
...
| V_CalcIntermissionRefdef | intermission branch of `view.calcRefdef` | VERIFIED | test_view: vieworg pinned to the view entity origin (no viewheight/bob), gunmodel 0 (view->model = NULL), ent angles + FULL idle sway (v_idlescale forced 1). | `lune run tests/test_view.luau` |
| V_CalcRefdef | view.calcRefdef | VERIFIED | test_view.luau composition battery: origin + viewheight + bob + 1/32 nudge, gun origin eye + forward*bob*.4 + z bob, punch added AFTER gun angles (C order), stair glide 80 u/s with the 12-unit cap and airborne snap, STAT_WEAPON/WEAPONFRAME pass-through. Deltas: pitch drift + V_BoundOffsets landed 2026-07-05 (see their rows); viewsize gun-z fudge landed 2026-07-07 (view.gunZFudge, +2 at the default 100, wired to sizeup/viewsize); still absent: player ent angles not forced to view. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | `lune run tests/test_view.luau` |
| V_RenderView | heartbeat camera CFrame + crosshair label | VERIFIED | The refdef->camera pipeline and the '+' conchars crosshair are visible in every committed live capture (S3 anchor onward); the refdef math itself is test_view. Crosshair default ON stands as the recorded divergence. Placement diverges from C's vrect-centre BY NECESSITY: the Roblox camera projects through the window centre (qcoords.vrect comment), so the glyph is anchored there — at the C vrect centre it sat 48px above the aim line and rockets landed below it (playtest report, fixed; evidence/nq-crosshair-aim.jpg). delta-ruled: ACCEPT (2026-07-07) — crosshair default ON is a documented deliberate default (FIDELITY.md) and the window-centre placement is an aim-line necessity. FIDELITY FIX 2026-07-07: the C pause gate (`if (!cl.paused) V_CalcRefdef()`) was missing — the view kept bobbing/drifting on a paused game; the heartbeat now freezes the last refdef while c.paused. | Play capture: RL shot at a far wall explodes under the '+'; `lune run tests/test_view.luau` |
...
| Sbar_Init | hud.create (wad pics cached lazily) | VERIFIED | Every committed sbar capture (S3 anchor, face/inventory battery) shows the wad pics loaded and laid out; lazy caching is the only delta from C's up-front Sbar_Init. delta-ruled: ACCEPT (2026-07-07) — port-wide lazy asset model. | Any Play capture with viewsize 100 |
...
| Sbar_DrawFace | hud face branch | VERIFIED | Health bands shown across committed captures: 100 (S3 anchor), 49 = band 3 ([nq-sbar-face49.jpg](evidence/nq-sbar-face49.jpg)), 5 = band 5 ([nq-sbar-face5.jpg](evidence/nq-sbar-face5.jpg)). Not yet shown visually: 0.2s pain frame and invis/invuln/quad specials (no artifacts near the e1m1 start) — formulas match C in code; re-stage on a powerup map to close. FIDELITY FIX 2026-07-07: hud.luau's IT_INVULNERABILITY/IT_QUAD bits were rotated (quad face showed on biosuit, invuln face on quad) — re-grounded against quakedef.h; the "formulas match C" claim above was true but the constants feeding them were not. | Stage per evidence/nq-sbar-faces.txt |
| Sbar_Draw | hud.update | VERIFIED | Full sbar layout (inventory row, armor/face/health/ammo bar) live in the battery captures and the S3 anchor. The old no-viewsize/lineadj delta is stale: hud.setViewsize implements the sb_lines gating (hud.luau:682-692 — ibar hidden at viewsize >= 110, canvas at >= 120), shown live in the SCR_SizeUp_f row's nq-sizeup-110.jpg evidence. | Stage per evidence/nq-sbar-faces.txt; S3 anchor diff |
...
| Sbar_MiniDeathmatchOverlay | hudlib `updateMiniOverlay` (NQ driver calls it in deathmatch, teamplay 0 columns) | VERIFIED | [evidence/nq-fragcells-minidm.jpg](evidence/nq-fragcells-minidm.jpg): the frags/name strip right of the sbar, self-windowed; re-anchors with viewsize ([evidence/nq-sizeup-110.jpg](evidence/nq-sizeup-110.jpg)). C gates kept (vid.width>=512, numlines>=3). | Stage per evidence/nq-fragcells-minidm.txt |
...
| SCR_DrawCenterString | hud.centerPrint rows + finale typewriter | VERIFIED | Per-line centered conchars drawing shown in [evidence/nq-centerprint.jpg](evidence/nq-centerprint.jpg). Finale typewriter reveal (scr_printspeed) captured: the Sbar_FinaleOverlay row's nq-finale-overlay.jpg closes the typewriter sub-aspect (the old not-yet-captured note was stale). | Stage per evidence/nq-centerprint.txt |
...
| Function | Port | Status | Evidence / Delta | How to verify |
|---|---|---|---|---|
| Con_ToggleConsole_f | console.toggle (backquote/tilde) | VERIFIED | [evidence/nq-console-open.jpg](evidence/nq-console-open.jpg) + .txt: console toggled open/closed through the harness across the battery. Key_ClearStates delta closed: refreshInputMode (init.client.luau:1470-1472) calls input.setEnabled(false) when the console opens, clearing held buttons and analog sticks (input.luau:145-154). | RQDBG_Console battery per evidence/nq-console-open.txt, capture, compare |
...
| Con_CheckResize | consolelib checkResize + hud notify rebuild | VERIFIED | con_linewidth = (width>>3)-3 at 16px chars, floor 38 like C; rows rebuilt when the count changes, checked every update tick; scrollback not re-flowed (C truncate-copy semantics). Live probe: 1244px viewport -> 74 cols on console AND notify rows ([evidence/con-checkresize.txt](evidence/con-checkresize.txt)). | Probe in the evidence file |
...
| Con_Print | console.printCon (open-line append; console.print terminates) | VERIFIED | [evidence/nq-console-open.jpg](evidence/nq-console-open.jpg) + .txt: scrollback renders the battery lines. 2026-07-07 fidelity fix: C's append-until-\n semantics ported — QC's pieced sprints ("You got the "+"nails\n") join into one line (user-reported split; live QW probe: "You got the Super Nailgun" and the 3-piece threewave team line render whole); width wrap now at live con_linewidth. Delta: 200-line scrollback vs 16K text buffer. | Pick up an item; the message renders on one notify line |
...
| Con_DrawInput | input row + blinking char-11 cursor | VERIFIED | [evidence/nq-console-open.jpg](evidence/nq-console-open.jpg) + .txt: ] prompt with blinking char-11 cursor. Delta: no horizontal scroll of long input. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | RQDBG_Console battery per evidence/nq-console-open.txt, capture, compare |
| Con_DrawNotify | hud notifyRows (4 lines, 3s) | VERIFIED | Conchar glyph census: +36 glyphs appear in the notify region after a server `say` print and expire at the 3s window (21 -> 57 -> 21, [evidence/nq-console-notify-clear.txt](evidence/nq-console-notify-clear.txt)); screenshots usually hide the area under Roblox chrome (recorded divergence: below-topbar inset) — but [evidence/nq-notify-thunderbolt.jpg](evidence/nq-notify-thunderbolt.jpg) catches a pickup notify line rendered in conchars. Port note: client `echo` prints console-only; notify rides server prints. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | Glyph census per the evidence file |
| Con_DrawConsole | console.update | VERIFIED | [evidence/nq-console-open.jpg](evidence/nq-console-open.jpg) + .txt: half-screen console over the world. Delta: no scrollback paging (con_backscroll; the Key_Console row carries the pgup/pgdn delta-open). The old no-version-string half was stale/wrong: WinQuake draws no version string at draw time — it is baked into conback at Con_Init, which the port does (Draw_CharToConback row). | RQDBG_Console battery per evidence/nq-console-open.txt, capture, compare |
...
| Function | Port | Status | Evidence / Delta | How to verify |
|---|---|---|---|---|
| Key_Console | console.handleKey/handleText | VERIFIED | The committed console battery (nq-console-open.jpg + nq-cbuf-battery.txt) was typed through consoleKey -> handleKey/handleText (RQDBG "key" action drives the real path; enter executes, backspace edits, history recalled). Tab completion exists (console.luau:201-206: Tab → com.completePrefix, wired both boots — the old no-completion note was stale). pgup/pgdn scrollback still absent. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). Clipboard delta-ruled: ACCEPT (2026-07-07) — platform-limited (no clipboard access). | RQDBG_Console "key"/"text" actions per the battery |
...
| Function | Port | Status | Evidence / Delta | How to verify |
|---|---|---|---|---|
| M_DrawCharacter / M_Print / M_PrintWhite | — | SUBSTITUTED | the pic pages substitute blits with ImageLabels; the old "no text menus yet" note is stale — menu.luau:134-140 loads a conchars confont and m.print exists; the Load/Save/Setup pages draw slot text (see the M_ScanSaves row's nq-load-menu.jpg) | — (substitution; verify justification still holds) |
...
| M_Menu_Main_f / M_Main_Draw / M_Main_Key | menu.create/update/handleKey | VERIFIED | [evidence/nq-main-menu.jpg](evidence/nq-main-menu.jpg): qplaque + ttl_main + mainmenu entries + animated menudot at the C coordinates. Key navigation since exercised with real key events (M_SinglePlayer and M_Keydown rows: Enter/Down/Return battery, nq-menu-cursor-help.jpg) — the old not-exercised note is stale. | Console "togglemenu" per evidence/nq-main-menu.txt, capture, compare |
...
| M_Draw | menu.update | VERIFIED | [evidence/nq-menu-cursor-help.jpg](evidence/nq-menu-cursor-help.jpg): MAIN plaque, QUAKE sidebar, id logo, item list and spinner cursor rendered mid-game. The old no-Draw_FadeScreen note is stale: menu.luau:111-117 draws the 0.5-alpha dim behind every page (the Draw_FadeScreen row is VERIFIED with the menu captures). | Studio: tools/verify_input_nq.luau menu steps + capture |
| M_Keydown | menu.handleKey | VERIFIED | Three real Down keys moved the spinner cursor to HELP/ORDERING ([evidence/nq-menu-cursor-help.jpg](evidence/nq-menu-cursor-help.jpg)); Return entered Help; M closed. The old no-menu-sounds note is stale: m.onBeep plays misc/menu1/2/3.wav on toggle, cursor moves and enters (menu.luau:329-476; the S_LocalSound row verified menu1.wav live). | Studio: tools/verify_input_nq.luau menu steps + captures |
...
| Draw_ConsoleBackground | conback ImageLabel in console.create | VERIFIED | The conback art fills the console in evidence/nq-console-open.jpg. Delta stands: the frame slides instead of cropping the pic. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | RQDBG_Console "toggle", capture |
...
| R_AliasSetupSkin | updateAlias skin select + player translation | VERIFIED | Skin selection feeds every alias capture; the translation table is C-truth tested (test_render_misc backwards-ranges battery). Delta stands: skingroup intervals not timed. delta-open: FIDELITY.md "Open" (2026-07-07 delta triage). | `lune run tests/test_render_misc.luau`; any monster capture |
...
| d_scan.c | — | SUBSTITUTED | span texture mapping; D_DrawTurbulent's water warp OUTPUT reproduced by textures.writeTurbFrame; D_WarpScreen (underwater full-screen warp) NOT reproduced — FIDELITY open item, no screen-space shader access. delta-ruled: ACCEPT (2026-07-07) — no screen-space shader access; FIDELITY.md Open #1 (approximation TBD) | — (substitution; verify justification still holds) |
...
| shared GUI image pages (font sheet + sbar pic page) | confont.luau / hud.luau / textures.newImagePage | platform constraint: the editable pool exhausts CUMULATIVELY per play session (black binds at 87 live images; dynamic-EM canary refused at 47k live verts; a fresh solid-red image bound to a black part stayed black) — fewer creations = slower burn. Root cause investigation open: tools/verify_editablepool.luau canary + protocol. The 2026-07-04 pow2 pad/resample commits were REVERTED after a controlled A/B showed non-pow2 renders fine on a healthy pool |
...
| Touch + gamepad controls (PRODUCT layer) | client/controls.luau + input.setAnalogMove/applyLook + controls.attach block in each boot | Roblox ships to phones/tablets/consoles; WinQuake has no counterpart (in_win.c is mouse/DirectInput only). Pure input frontend: on-screen thumbstick + FIRE/JUMP/next-weapon/SCORES buttons and controller bindings all route through the same bind-exec path as keys (+attack, impulse 10, togglemenu) plus two analog hooks folded into CL_BaseMove sums; sticks = run speed at full deflection (server sv_maxspeed clamps), partial = walk. Gamepad: R2 fire, A/L2 jump, R1/Y/L1/DPad weapon cycle, Select scores, Start menu, DPad/A/B navigate the Quake menu. Touch UI auto-hides on keyboard+mouse desktops and while a controller is active; Players.CharacterAutoLoads=false keeps Roblox's own TouchGui away. Engine core untouched. Verified live both boots (2026-07-06): analog move 419qu run + 88qu/s walk at 40% stick, look exact 90 deg, fire via sound census + shells 25->21, jump apex 43qu on pad and touch paths, axe/shotgun impulse cycle, console-open gating, RQDBG_Controls/RQDBG_ForceTouchUI harness. |
| Roblox avatars mode (PRODUCT substitution, toggleable) | render/avatarrender.luau + init.client relink branch + init.server rig publisher | roblox_avatars attribute (live-flippable): player entities render as the player's own R15 avatar rig (server publishes neutralized rigs under ReplicatedStorage.QuakeAvatars; client clones, drives position/yaw from the entity, idle/run anims by measured speed; held weapon mirrored via the RQWeaponModel player attribute and attached to the right hand as the g_* world model). Engine core untouched: wire protocol unchanged; the alias renderer remains the fallback (rig loading, player.mdl death frames 41-102, toggle off). NQ boot only; QW parity pending. |
| host-command gate + say suppression (PRODUCT policy hooks) | host.luau HOST_ONLY set + svr.commandGate / svr.suppressSay; init.server policy | real NetQuake host commands are local-console-only; the Roblox mapping is Studio/private-server-owner/place-creator. Player speech rides TextChatService (filtered) instead of the wire; suppressSay drops raw say server-side. No hooks installed offline = listen-server semantics (loopback tests unchanged). |
